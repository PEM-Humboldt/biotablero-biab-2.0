...
load_libraries(c("doParallel","sf","dplyr"))
...
geo_defined = c("basin_areas", "basin_subzones", "basin_zones", "environmental_authorities", "states")
...
forest_results = foreach(i = 1:length(raster_file), .combine = rbind.data.frame, .verbose = TRUE) %do% {
  ...
  # Delimit results to geofences with data if no data were obtained
  if (geo_ids != "_"){
